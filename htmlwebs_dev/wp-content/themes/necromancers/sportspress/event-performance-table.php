...
">
  <div class="sp-table-wrapper">
    <table class="matches-table lineups-table lineups-table--<?php echo esc_attr( $layout ); ?> sp-event-performance sp-data-table<?php if ( $mode == 'values' ) { ?><?php if ( $scrollable ) { ?> sp-scrollable-table<?php }if ( $responsive ) { echo ' sp-responsive-table ' . $identifier; } if ( $sortable ) { ?> sp-sortable-table<?php } ?><?php } ?>">
      <thead>
        <tr>
          <?php if ( $mode == 'values' ) : ?>
            <?php if ( $show_players ) : ?>
      ...
      <?php if ( $show_players ): ?>
        <tbody>
          ...
          foreach ( $data as $player_id => $row ) :
            ...
            $name = $player_name = get_the_title( $player_id );
            ...
            if ( has_post_thumbnail( $player_id ) ) {
              ...
              $player_photo = '<img src="' . get_template_directory_uri() . '/assets/img/placeholders/placeholder-player-60x60.png" class="match-player__placeholder" alt="' . esc_attr( get_the_title( $player_id ) ) . '" />';
            }
            ...
            if ( $stars_type ):
              ...
              if ( $player_stars ) :
                switch ( $stars_type ) :
                  case 1:
                    $stars_output = ' <span class="sp-event-stars"><i class="sp-event-star dashicons dashicons-star-filled" title="' . esc_attr__( 'Player of the Match', 'necromancers' ) . '"></i></span>';
                    break;
                  case 2:
                    $stars_output = ' <span class="sp-event-stars">' . str_repeat( '<i class="sp-event-star dashicons dashicons-star-filled" title="' . esc_attr__( 'Stars', 'necromancers' ) . '"></i>', $player_stars ) . '</span>';
                    break;
                  case 3:
                    $stars_output = ' <span class="sp-event-stars"><i class="sp-event-star sp-event-star-' . $player_stars . '  dashicons dashicons-star-filled" title="' . esc_attr__( 'Stars', 'necromancers' ) . '"></i><span class="sp-event-star-number">' . $player_stars . '</span></span>';
                    break;
                endswitch;
              endif;
            endif;

            $name = '<figure class="match-player ncr-team-id-' . $team_id . '" role="group">
              <figure class="match-player__avatar">' . $player_photo . '</figure>
              <figcaption>
                <span class="match-player__nickname">' . $name . $stars_output . '</span>
            ...
            foreach ( $labels as $key => $label ):
              ...
              elseif ( intval( $value ) && $mode == 'icons' ) :
                ...
                $content .= '<span class="ncr-performance">' . apply_filters( 'sportspress_event_performance_icons', $icons, $performance_id, 1 ) . '<span class="ncr-performance__value">' . $value . '</span></span>';
              endif;
            endforeach;

            if ( 'layout-3' === $layout ) {

              // Output Player and Hero (for icons)
              if ( isset( $position ) && $mode == 'icons' ):
                $name .= ' <small class="sp-player-position">' . $position . '</small>';
              endif;

              echo '<td class="data-name" data-label="' . ( isset( $section_label ) ? $section_label : esc_attr__( 'Player', 'necromancers' ) ) .'">' . $name . '</td>';
            } else {

              // Output Hero
              echo '<td class="data-name" data-label="' . esc_attr__( 'Hero', 'necromancers' ) .'">';
                echo '<figure class="ncr-team-id-' . $team_id . ' match-player match-player--small">';
                  echo '<figure class="match-player__avatar">';
                    echo wp_get_attachment_image( $positions[0]['image'], 'necromancers-sp-fit-icon-sm' );
                  echo '</figure>';
                  echo '<figcaption><span class="match-player__nickname">' . $player_name . $stars_output . '</span><span class="match-player__name">' . $positions[0]['role'] . '</span></figcaption>';
                echo '</figure>';
              echo '</td>';
            }
          ...
          if ( $show_total && ( ! $primary || sizeof( array_intersect_key( $totals, array_flip( (array) $primary ) ) ) ) ) {
            ...
            <tr class="sp-total-row <?php echo ( $i % 2 == 0 ? 'odd' : 'even' ); ?>">
              ...
              if ( $show_players ) :
                ...
                if ( $mode == 'values' ) :
                  echo '<td class="data-name" data-label="&nbsp;">' . esc_attr__( 'Total', 'necromancers' ) . '</td>';
                endif;
              endif;
