		...
		if ( empty( $group->term_id ) || has_term( $group->term_id, 'sp_position', $player_id ) ) :
			...
			if ( $show_player_photo ) :
				if ( has_post_thumbnail( $player_id ) ) :
					$logo        = get_the_post_thumbnail( $player_id, 'necromancers-sp-fit-icon-sm' );
...
endforeach;
	...
	$output .= '<div class="sp-table-wrapper">' .
		'<table class="sp-player-list sp-data-table' . ( $sortable ? ' sp-sortable-table' : '' ) . ( $responsive ? ' sp-responsive-table ' . $identifier : '' ) . ( $scrollable ? ' sp-scrollable-table' : '' ) . ( $paginated ? ' sp-paginated-table' : '' ) . ( $custom_class ? ' ' . $custom_class : '' ) . '" data-sp-rows="' . $rows . '">';
