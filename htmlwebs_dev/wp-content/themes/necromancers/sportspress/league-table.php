...
$output .= '<table class="table matches-table standings-table' . ( $sortable ? ' sp-sortable-table' : '' ) . ( $responsive ? ' sp-responsive-table '.$identifier : '' ). ( $scrollable ? ' sp-scrollable-table' : '' ) . ( $paginated ? ' sp-paginated-table' : '' ) . '" data-sp-rows="' . $rows . '">' . '<thead>' . '<tr>';
...
$output .= '<th class="data-rank">' . esc_html__( 'Pos', 'necromancers' ) . '</th>';

foreach( $labels as $key => $label ):
  if ( ! is_array( $columns ) || $key == 'name' || in_array( $key, $columns ) ) {
    if ( $key == 'name' && sp_get_post_mode( $id ) == 'player' ) {
      $output .= '<th class="data-' . $key . '">' . esc_html__( 'Player', 'necromancers' ) . '</th>';
    } else {
...
foreach ( $data as $team_id => $row ):
  ...
  if ( $show_team_logo ) :
    if ( has_post_thumbnail( $team_id ) ) :
      $logo = get_the_post_thumbnail( $team_id, 'sportspress-fit-mini' );
      $name = '<span class="match-team-logo ncr-team-id-' . esc_attr( $team_id ) . '">' . $logo . '</span>' . '<div class="match-team__desc"><div class="match-team__name">' . $name . '</div>' . '<div class="match-team__country">' . $origin . '</div></div>';
      $name_class .= ' has-logo';
    endif;
  endif;
  ...
  if ( $link_posts ) {
    ...
    $name = '<a href="' . $permalink . '" class="match-team match-team--v-align">' . $name . '</a>';
  } else {
    $name = '<figure class="match-team match-team--v-align">' . $name . '</figure>';
  }
...
if ( $show_full_table_link )
  $output .= '<div class="sp-league-table-link sp-view-all-link"><a href="' . get_permalink( $id ) . '">' . esc_html__( 'View full table', 'necromancers' ) . '</a></div>';
?>
