...
class Necromancers_Off_Canvas_Nav extends Walker_Nav_Menu {
  ...
  function start_lvl(&$output, $depth = 0, $args = array()) {
    ...
    if ( $this->active_megamenu ) {
      if ( $depth === 0 ) {
        $output .= "\n$indent<ul class=\"dl-megamenu dl-submenu dl-submenu__sub-" . $depth . " {locate_class}\">\n";
      } else {
        $output .= "\n$indent<ul class=\"{locate_class}\">\n";
      }
    } else {
      $output .= "\n$indent<ul class=\"dl-submenu dl-submenu__sub-" . $depth . " {locate_class}\">\n";
    }
  }
  ...
  function end_lvl(&$output, $depth = 0, $args = array()) {
    ...
    if ( $depth === 0 ) {
      if ( $this->active_megamenu ) {
        $output = str_replace("{locate_class}", "dl-megamenu-item dl-megamenu-col-" . $this->max_columns . "", $output );

        foreach ( $this->rowsCounter as $row => $columns ) {
          $output = str_replace("{current_row_" . $row . "}", "dl-megamenu-item col dl-megamenu-col-" . $columns, $output);
        }
    ...
    if ($depth === 1 && $this->active_megamenu) {
      ...
      if ( $this->megamenu_widgetarea == false ) {
        ...
        if ($title != "&#8211;" && $title != '"&#8211;"' && $title != "-" && $title != "&#8212;") {
          ...
          $item_output .= '<h6 class="dl-menu__title"' . $attributes . '>';
