...
if(!class_exists("DFWPThemeActivationBase")) {
	class DFWPThemeActivationBase {
    	public $key = "1AC86209520B7659";
    	private $product_id = "9";
    	private $product_base = "necromancers";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_necromancers_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=necromancers_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
