	...
	public static function init() {
		...
		self::$fields = array(
			array(
				'key'		      => 'megamenu',
				'type'		    => 'checkbox',
				'label' 	    => esc_html__( 'Mega Menu?', 'necromancers' ),
				'description' => ''
			),
			array(
				'key'		      => 'megamenu_widgetarea',
				'type'		    => 'widget_area',
				'label' 	    => esc_html__( 'Mega Menu Widget Area', 'necromancers' ),
				'description' => ''
			),
		);
	}
		...
		$field_value = ($value != "") ? "checked='checked'" : '';
			...
			<p class="description description-wide <?php echo esc_attr( $class ); ?>">
				<label for="<?php echo esc_attr( $id ); ?>">
					<input type="checkbox" value="enabled" class="edit-menu-item-necromancers-megamenu-check" id="<?php echo esc_attr( $id ); ?>" name="<?php echo esc_attr( $name ); ?>" <?php echo esc_html( $field_value ); ?> />
		...
		<p class="description description-wide <?php echo esc_attr( $class ) ?>">
			<label for="<?php echo esc_attr( $id ); ?>">
				...
				<select id="<?php echo esc_attr( $id ); ?>" class="widefat code <?php echo esc_attr( $id ); ?>" name="<?php echo esc_attr( $name ); ?>">
					<option value="0"><?php esc_html_e( 'Select Widget Area', 'necromancers' ); ?></option>
					<?php
