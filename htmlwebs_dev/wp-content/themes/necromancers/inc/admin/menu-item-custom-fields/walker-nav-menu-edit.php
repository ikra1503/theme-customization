...
class Menu_Item_Custom_Fields_Walker extends Walker_Nav_Menu_Edit {
	...
	/**
   * Get the preg_replace pattern.
   *
   * Check current WP version and return the suitable pattern.
   *
   * @access private
   * @since 0.1.0
   *
   ...
   */
  private function get_replace_pattern() {

    global $wp_version;

    // NOTE: Check this regex from time to time!
    if ( version_compare( $wp_version, '4.7', '>=') ) {
      return '/(?=<fieldset[^>]+class="[^"]*field-move)/';
    }

    return '/(?=<p[^>]+class="[^"]*field-move)/';
  }
