...
class Necromancers_Nav_Menu extends Walker_Nav_Menu {
  ...
  function start_lvl(&$output, $depth = 0, $args = array()) {
    ...
    if ( $this->active_megamenu ) {
      if ( $depth === 0 ) {
        $output .= "\n$indent<ul class=\"main-nav__megamenu main-nav__sub-" . $depth . " {locate_class}\">\n";
      } else {
        $output .= "\n$indent<ul class=\"{locate_class}\">\n";
      }
    } else {
      if ( $depth > 0 ) {
        $output .= "\n$indent<ul class=\"main-nav__sub main-nav__sub--dropup main-nav__sub-" . $depth . " {locate_class}\">\n";
      } else {
        $output .= "\n$indent<ul class=\"main-nav__sub main-nav__sub-" . $depth . " {locate_class}\">\n";
      }
    }
  }
    ...
    if ( $depth === 0 ) {
      if ( $this->active_megamenu ) {
        ...
        foreach ( $this->rowsCounter as $row => $columns ) {
          $output = str_replace("{current_row_" . $row . "}", "megamenu-item col megamenu-col-" . $columns, $output);
        }
    ...
    if ($depth === 1 && $this->active_megamenu) {
      ...
      if ( $this->megamenu_widgetarea == false ) {
        ...
        if ($title != "&#8211;" && $title != '"&#8211;"' && $title != "-" && $title != "&#8212;") {
          ...
          $item_output .= '<h6 class="main-nav__title"' . $attributes . '>';
