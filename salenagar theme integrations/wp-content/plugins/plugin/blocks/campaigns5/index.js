( function( wp ) {
	 ...
	 * @see https://wordpress.org/gutenberg/handbook/designers-developers/developers/block-api/#registering-a-block
	 */
	registerBlockType( 'lifeline-donation/campaigns5', {
		/**
		 * This is the display title for your block, which can be translated with `i18n` functions.
		 * The block inserter will show this name.
		 */
		title: __( 'Donation Campaigns 5', 'lifeline-donation' ),
		description: __( 'Campaigns style 5: you can show donation button by displaying different content and banners to attract donors', 'lifeline-donation' ),
		...
		edit: withSelect(function(select, props){
			...
			return [
					...
					el(
						'section',
						{className: 'wpcm-wrapper urgent-popup-list wp-block-lifeline-donation-campaigns5'},
						(posts) ? 
							el(
								'div', {className: 'wpcm-causes-style5'},
								el(
									'div', {className: 'wpcm-row'},
									el(
										'div', {className: 'wpcm-col-lg-4 wpcm-col-md-6'},
										el(
											'div', {className: 'wpcm-cause-style5'},
											el(
												'div', {className: 'wpcm-cause-style5-img'},
												el(
													'figure', {},
													el('img', {src: (media) ? ((media[0]) ? media[0].source_url : '') : ''})
												),
												el(
													'div', {className: 'wpcm-cause-style5-contnt'},
													el(
														'div', {className: 'wpcm-cause-style5-info'},
														el('a', {href:'#'}, __('Water', 'lifeline-donation')),
														el('h2', {}, first(posts).title.rendered),
														el(
															'span', {}, __('Raised', 'lifeline-donation'),
															el('strong', {}, '$1900000')
														)
													),
													el(
														'div', {className: 'wpcm-cause-style5-meta'},
														el(
