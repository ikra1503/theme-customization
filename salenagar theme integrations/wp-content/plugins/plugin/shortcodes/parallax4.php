...
class Parallax4 extends Shortcodes
{
	static function init() {
		self::$shortcode = 'wi_donation_parallax4';
		self::$vc_map = self::vc_map();
		parent::init();
		add_filter('vc_autocomplete_wi_donation_parallax4_post_id_callback', array(__CLASS__, 'field_search'), 10, 3);
		add_filter('vc_autocomplete_wi_donation_parallax4_post_id_callback', array(__CLASS__, 'field_search'), 10, 3);
	}

	static function vc_map() {
		return [
			"name" => esc_html__("Parallax Style 4", 'lifeline-donation-pro'),
            "base" => self::$shortcode,
