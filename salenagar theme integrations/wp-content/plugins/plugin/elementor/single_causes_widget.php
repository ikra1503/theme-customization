 ...
 * @since 1.0.0
 */
class Single_Causes_Widget extends \Elementor\Widget_Base {
	...
	public function get_name() {
		return 'wi_donation_single_causes_widget';
	}
	...
	public function get_title() {
		return esc_html__( 'Single Cause Widget', 'lifeline-donation-pro' );
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			'widget_title',
			[
				'label'   => esc_html__( 'Widget Title', 'lifeline-donation-pro' ),
		...
		$this->add_control(
			'button_icon',
			[
				'label' => esc_html__( 'Show Icon', 'lifeline-donation-pro' ),
				'type'  => Controls_Manager::SWITCHER,
			]
		);
		$this->add_control(
		...
		/**
		 * Staring section for misc settings.
		 */
		$this->start_controls_section(
			'misc_settings',
			[
				'label' => esc_html__( 'Misc', 'lifeline-donation-pro' ),
			]
		);
		...
		$this->add_control(
			'show_raised_amt',
			[
				'label' => esc_html__( 'Show Raised Amount', 'lifeline-donation-pro' ),
				'type'  => Controls_Manager::SWITCHER,
			]
		);
		$this->add_control(
			'raised_amount_label',
			[
				'label'     => esc_html__( 'Raised Amount Label', 'lifeline-donation-pro' ),
				'type'      => Controls_Manager::TEXT,
				'default'   => esc_html__( 'Raised', 'lifeline-donation-pro' ),
				'condition' => [
					'show_raised_amt' => 'yes',
				],
			]
		...
		$this->add_control(
			'progress_bar',
			[
				'label'   => esc_html__( 'Show Progressbar', 'lifeline-donation-pro' ),
				'type'    => Controls_Manager::SWITCHER,
				'default' => 'yes',
			]
		);
		$this->add_control(
			'progress_bar_color',
			[
				'label'     => esc_html__( 'Progressbar color', 'lifeline-donation-pro' ),
				'type'      => Controls_Manager::COLOR,
				'default'   => '#0d9b8c',
				'condition' => [
					'progress_bar' => 'yes',
				],
				'selector'  => '{{WRAPPER}} .progress .progress-bar',
			]
		);
	...
	protected function render() {
		...
		$this->add_render_attribute( 'wrapper', 'class', 'wpcm-wrapper lifeline-donation-app wpcm-widget wpcm-urgnt-causes-widget' );
		$this->add_render_attribute( 'button', 'class', 'elementor-button wpcm-btn wpcm-btn-border' );
		...
		if ( $query->have_posts() ) {
			...
			$file = get_theme_file_path( 'templates/elementor/single_causes_widget.php' );

			if ( file_exists( $file ) ) {
				include $file;
			} else {
				include LIFELINE_DONATION_PATH . 'elementor/output/single_causes_widget.php';
			}
		}
		wp_reset_postdata();
	}
