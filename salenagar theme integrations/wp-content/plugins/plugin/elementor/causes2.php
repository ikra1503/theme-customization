 ...
 * @since 1.0.0
 */
class Causes2 extends \Elementor\Widget_Base {
    ...
    public function get_title() {
        return esc_html__( 'Donation Causes 2', 'lifeline-donation-pro' );
    }
    ...
    protected function render() {
        ...
        $this->add_render_attribute('button', 'class', 'wpcm-btn elementor-button wpcm-btn-border');

        if( $settings->get('button_icon') ) {
            $this->add_render_attribute('button', 'class', 'wpcm-btn-icon');
        }
        ...
        if( $query->have_posts() ) {
            ...
            $file = get_theme_file_path( 'templates/elementor/causes.php' );

            if( file_exists($file)) {
                include $file;
            } else {
                include LIFELINE_DONATION_PATH . 'elementor/output/causes2.php';
            }
        }
        wp_reset_postdata();
    }
